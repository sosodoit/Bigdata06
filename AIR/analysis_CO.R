...
f.co <- function(tr, te){
  ...
  #                            1. 시계열 데이터 변환
  #******************************************************************************#
  # msts <- msts(train.co.xts, seasonal.periods = c(7,365.25), start = decimal_date(as.Date("2010-01-01")))
  # plot(msts, main="Weekly co", xlab="Week", ylab="Weekly co")
  ...
  # train.co$week <<- as.integer(train.co$week)
  # mod_gam <- gam(co ~ s(week), data = train.co, method = "REML")
  # summary(mod_gam)
  # par(mfrow=c(2,2))
  # gam.check(mod_gam)
  # par(mfrow=c(1,1))
  # ggGam(mod_gam)
  # a=predict(mod_gam, newdata=test.co$co)
...
melt_data <- melt(train.gangnam, id.vars = c("co", "O3","CO","SO2","co"))
...
save.image(file = "./analysis_CO_sgg39.RData")
