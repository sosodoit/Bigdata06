...
source('sgg_separate.R', encoding='utf-8')     # week data
...
f.pm10 <- function(sgg){
  train.pm10 <- sgg[,c(2,3)]
  test.pm10 <- sgg.te[,c(2,3)]
}
...
#******************************************************************************#
# train
train.pm10.xts <- xts(train.pm10$pm10, order.by=train.pm10$DATE, frequency = freq)

# test
test.pm10.xts <- xts(test.pm10$pm10, order.by=test.pm10$DATE, frequency=freq)

# ts <- xts
train.pm10.ts <- ts(train.pm10.xts,start=c(2010,1), frequency= 365)
test.pm10.ts <- ts(test.pm10.xts, start=c(2020,1), frequency= 365)
#******************************************************************************#
# 체크용
# require(graphics)
# print(ts(test.pm10.xts, frequency=freq), calendar = TRUE)
#******************************************************************************#
#                          2. 모형 전 시계열 데이터 진단
#                           --> 통계적으로 적절하지 않다고 나옴
#******************************************************************************#
autoplot(train.pm10.ts)
...
ggAcf(train.pm10.ts) + ggtitle("s1.pm10 자기상관함수(ACF)")
Box.test(train.pm10.ts, lag = 24, fitdf = 0, type = "Lj")
#******************************************************************************#
#                           2.1 자기상관함수 시각화
#******************************************************************************#
acf(train.pm10.ts, main = "자기상관함수", col = "red")
pacf(train.pm10.ts, main = "부분자기상관함수", col = "blue")
...
#                               기본 시계열 분해                               #
#******************************************************************************#
m <- decompose(train.pm10.ts)
attributes(m)
plot(m)
plot(train.pm10.ts - m$seasonal) # 계절성 제거된 그래프
par (new = TRUE)
plot(train.pm10.ts - m$trend, col = "blue") # 추세요인 제거 그래프
plot(train.pm10.ts - m$seasonal - m$trend) # 불규칙 요인만 출력
#******************************************************************************#
#                                 차분 시각화 
#******************************************************************************#
plot(diff(train.pm10.ts, differences = 1), main="차분")
par (new = TRUE)
plot(diff(train.pm10.ts, differences = 12), col=2)
...
#******************************************************************************#
# 벤치마크 시계열 예측모형
train.pm10.ts %>% naive() %>% checkresiduals()
train.pm10.ts %>% naive() %>% forecast() %>% autoplot()

### 1. ETS 모형
train.pm10.ts %>% ets(lambda=BoxCox.lambda(train.pm10.ts)) %>% checkresiduals()
train.pm10.ts %>% ets(lambda=BoxCox.lambda(train.pm10.ts)) %>% forecast() %>% autoplot()

### 2. ARIMA 모형
train.pm10.ts %>% auto.arima() %>% checkresiduals()
train.pm10.ts %>% auto.arima() %>% forecast() %>% autoplot()

### 3. TBATS 모형
train.pm10.ts %>% tbats() %>% checkresiduals()
train.pm10.ts %>% tbats() %>% forecast() %>% autoplot()
#******************************************************************************#
#                           pm10 예측모형 선정과 예측
#******************************************************************************#
# 구별 for문 
...
mod_lst <- list (
  ...
  mod_exponential = ets(train.pm10.ts, ic='aicc', restrict=FALSE),
  mod_neural = nnetar(train.pm10.ts),
  mod_tbats = tbats(train.pm10.ts, ic='aicc', seasonal.periods=c(24,24*7)),
  mod_bats = bats(train.pm10.ts, ic='aicc', seasonal.periods=c(24,24*7)),
  mod_sts = StructTS(train.pm10.ts),
  mod_stl = stlm(train.pm10.ts, ic='aicc', robust=TRUE, method='ets')
)
...
train.pm10$date <- as.integer(train.pm10$DATE)
mod_gam <- gam(pm10 ~ s(date), data = train.pm10, method = "REML")
summary(mod_gam)
par(mfrow=c(2,2))
gam.check(mod_gam)
par(mfrow=c(1,1))
...
acc <- lapply(forecasts, function(f){
  accuracy(f, test.pm10.ts)[2,,drop=FALSE]
})

acc <- do.call(rbind, acc)
row.names(acc) <- names(forecasts)
acc <- acc[order(acc[,'MASE']),] %>% round(2)
# 실제값(test.pm10.ts)과 예측값(forecasts) 저장 => 구별 물질1개 => 분포보기

#******************************************************************************#
##                              모형적합 및 예측
#******************************************************************************#
train.pm10.ts %>% StructTS() %>% checkresiduals()
train.pm10.ts %>% StructTS() %>% forecast(h=5) %>% autoplot() +
  labs(x="", y="pm10", title="sgg1 pm10 향후 5년 예측")

train.pm10.ts %>% StructTS() %>% forecast(h=5) %>% 
  as_tibble() %>% 
  DT::datatable() %>% 
...
#                           모형 진단(모형 타당성 검정)
#******************************************************************************#
diff = diff(train.pm10.ts)
auto.arima(train.pm10.ts)
model <- arima(train.pm10.ts, order = c(0,1,1), seasonal = list(order = c(0,0,2))) #####확인
model
#******************************************************************************#
# -1) 자기상관함수에 의한 모형 진단
tsdiag(model)
...
par(new = TRUE)
plot(test.pm10.ts, xlim = c(2009,2022),ylim = c(0,0.08), col = "red") ####################왜지...
#******************************************************************************#
# 정상성 시계열의 계절형 : 분해하는건 똑같아보이는데.. 그래프가 좀 다르게나옴
ts_feature <- stl(train.pm10.ts, s.window = "periodic")
plot(ts_feature)
#******************************************************************************#
#                                 예측정확도
#                   https://otexts.com/fppkr/accuracy.html
#******************************************************************************#
# train.pm10.ts <- ts(train.pm10.xts, start = c(2009,12,28), frequency=freq)
# test.pm10.ts <- ts(test.pm10.xts, start = c(2020,1,6), end=c(2020,11,25), frequency=freq)
...
#******************************************************************************#
seasonplot(train.pm10.ts,
           year.labels=TRUE, year.labels.left=TRUE, col=1:20, pch=19)

monthplot(train.pm10.ts) #에 그림이 바뀜..
